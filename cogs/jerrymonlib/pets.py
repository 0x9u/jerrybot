...
class JerryMonPets(JerryMonCore):
    @JerryMonCore.jerrymon_group.command(name="get_free", description="Get free jerrymon (you must not have any).")
    async def get_free(self, interaction: discord.Interaction):
        ...
        await database.db.add_jerrymon_to_inventory(str(interaction.user.id), random_jerrymon)
        jerrymon = await database.db.get_jerrymon_by_id(random_jerrymon)
        await interaction.followup.send(f"You got a {jerrymon['name']}!")

    @JerryMonCore.jerrymon_group.command(name="party", description="List jerrymon you currently have on you.")
    ...
    @JerryMonCore.jerrymon_group.command(name="box", description="List jerrymon in storage.")
    ...
    @JerryMonCore.jerrymon_group.command(name="nickname", description="Nickname your jerry.")
    ...
    async def nickname(self, interaction: discord.Interaction, jerrymon_id: int, nickname: str):
        ...
        await database.db.nickname_jerrymon(str(interaction.user.id), jerrymon_id, nickname)
        await interaction.followup.send(f"Nickname set to {nickname} for ID: {jerrymon_id}.")

    @JerryMonCore.jerrymon_group.command(name="abandon", description="Abandon your jerry.")
    ...
    @JerryMonCore.jerrymon_group.command(name="info", description="Get information on a jerrymon.")
    ...
    async def info(self, interaction: discord.Interaction, jerrymon_name: str):
        ...
        embed = discord.Embed(
            ...
            (f"\nEvolves to {jerrymon_evolution['name']} at level {
             jerrymon['evolution_level']}."
             if jerrymon_evolution else ""),
            color=discord.Color.blue(),
        )
        await interaction.followup.send(embed=embed)

    @JerryMonCore.jerrymon_group.command(name="mine", description="Get information on your jerrymon.")
    async def mine(self, interaction: discord.Interaction, jerrymon_id: int):
        await interaction.response.defer()

        await database.db.verify_user(str(interaction.user.id))
        # todo: use jerrymons embeded
        jerrymon = await database.db.get_jerrymon_inventory_by_id(str(interaction.user.id), jerrymon_id)
        if not jerrymon:
            await interaction.followup.send("You don't have that jerrymon.")
            return

        jerrymon_data = await database.db.get_jerrymon_by_id(jerrymon["jerrymon_id"])

        jerrymon_evolution = await database.db.get_jerrymon_by_id(jerrymon_data["evolution_id"]) if jerrymon_data["evolution_id"] else None

        embed = discord.Embed(
            title=f"Name: {jerrymon['nickname']
                           or jerrymon_data['name']}",
        ...
        embed.add_field(
            name="Moves",
            value=f"{'\n'.join([f"Name: {move['jerrymons_moves']['name']}]\n"
                                f"Description: {
                move['jerrymons_moves']['description']}\n"
                f"Type: {
                get_jerrymon_status_condition(move['jerrymons_moves']['type'])
                if move['jerrymons_moves']['category'] == JerrymonMoveCategory.Status.value
                else get_jerrymon_type(move['jerrymons_moves']['type'])
            }\n"
                f"Category: {
                get_jerrymon_move_category(move['jerrymons_moves']['category'])
            }\n"
                f"Power: {
                move['jerrymons_moves']['power']}\n"
                f"Accuracy: {
                move['jerrymons_moves']['accuracy']}\n"
                f"MP cost: {
                move['jerrymons_moves']['mp']}"
                for move in moves])}")
        ...
        if jerrymon_evolution:
            embed.add_field(
                ...
                f"Level: {jerrymon_evolution['evolution_level']}\n"
    ...
    @JerryMonCore.jerrymon_group.command(name="give", description="Give your jerrymon to another player.")
    ...
    @JerryMonCore.jerrymon_group.command(name="add_party", description="Add jerrymon to party.")
    @app_commands.describe(jerrymon_id="Id of jerrymon.")
    async def add_party(self, interaction: discord.Interaction, jerrymon_id: int):
        ...
        party_len = await database.db.get_jerrymon_party_count(str(interaction.user.id))
        if party_len >= 6:
    ...
    @JerryMonCore.jerrymon_group.command(name="remove_party", description="Remove jerrymon from party.")
    @app_commands.describe(jerrymon_id="Id of jerrymon.")
