...
def jerrymon_calculate_damage(jerrymon_attack: int, move_power: int, opponent_defense: int) -> int:
    scaled_attack = (jerrymon_attack ** 1.5) / 10
    damage = ((scaled_attack * move_power / (opponent_defense + 1)) / 50) + 5
    return max(1, int(damage))

def jerrymon_calculate_xp_earnt(jerrymon_level: int, other_jerrymon_level: int) -> int:
    return max(JERRYMON_BASE_XP, (jerrymon_level - other_jerrymon_level) * JERRYMON_BASE_XP)
